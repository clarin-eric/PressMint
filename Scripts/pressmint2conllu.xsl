...
<!-- Convert PressMint.ana to CoNLL-U:
     - document (text), paragraph (p) and sentence (s) IDs
     - syntactic (or modernised) words (w/w)
     - XPoS (w/@ana)
...
<xsl:stylesheet version='2.0' 
  ...
  <!-- A text corresponds to a document -->
  <xsl:template match="tei:text">
      <xsl:value-of select="concat('# newdoc id = ', ancestor::tei:TEI/@xml:id, '&#10;')"/>
      <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template match="tei:text//tei:p">
    ...
    <xsl:choose>
      <xsl:when test="tei:s">
        <xsl:if test="not(normalize-space($p-lang)) or $lang = $p-lang">
      ...
      <xsl:otherwise>
        <xsl:message select="concat('WARN: skipping paragraph without sentences ', @xml:id)"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
