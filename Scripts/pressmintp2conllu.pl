#!/usr/bin/env perl
# Convert PressMint .ana files to CoNLL-U and validate them
...
sub usage
{
    print STDERR ("Usage: pressmintp2conllu.pl -jobs <Jobs> -in <InputDirectory> -out <OutputDirectory>\n");
...
foreach $inFile (glob($corpusFiles)) {
    if ($inFile =~ m|PressMint-[A-Z]{2}(?:-[A-Z0-9]{1,3})?(?:-[a-z]{2,3})?\.ana\.xml|) {$rootAnaFile = $inFile}
    elsif ($inFile =~ m|PressMint-[A-Z]{2}(?:-[A-Z0-9]{1,3})?(?:-[a-z]{2,3})?_.+\.ana\.xml|) {push(@compAnaFiles, $inFile)}
}
my ($country, $MT) = $rootAnaFile =~ /PressMint-([A-Z]{2}(?:-[A-Z0-9]{1,3})?)(?:-([a-z]{2,3}))?\.ana\.xml/
...
if (defined $MT) {$langs = $MT}
    ...
    die "FATAL ERROR: Can't find mapping between country code and language(s): ".
        "pls. add \$country2lang{'$country'} to pressmintp2conllu.pl!\n"
}
