<?xml version="1.0"?>
<!-- Take template for root PressMint corpus file and add info from XIncluded roots -->
<xsl:stylesheet 
  ...
  <!-- Directory relative to location of this script, where the PressMint corpora are found -->
  <xsl:param name="base"/>
  <xsl:param name="type"/>
  ...
  <xsl:variable name="corpusType" select="concat('PressMint:',$type)"/>
  <xsl:variable name="corpusTypeSuf">
    <xsl:choose>
      <xsl:when test="$corpusType = 'PressMint:TEI'"></xsl:when>
      <xsl:when test="$corpusType = 'PressMint:TEI.ana'">.ana</xsl:when>
      <xsl:when test="$corpusType = 'PressMint:en.TEI.ana'">-en.ana</xsl:when>
      <xsl:otherwise><xsl:message terminate="yes">Invalid or missing type '<xsl:value-of select="$type"/>'</xsl:message></xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  <xsl:variable name="corpusId" select="concat('PressMint',$corpusTypeSuf)"/>

  <xsl:variable name="docs">
    <xsl:choose>
      <xsl:when test="$isSample = '1'">
        <xsl:variable name="all-files">
          <xsl:for-each select="uri-collection(concat($base, '/?select=PressMint-*;recurse=yes'))">
  ...
  <xsl:template match="@xml:id[. = 'PressMint-rootTemplate']">
  ...
  <xsl:template match="@corresp">
    <xsl:variable name="val" select="normalize-space(replace(concat(' ',.),' PressMint:[^ ]*',''))"/>
    <xsl:if test="$val">
  ...
  <xsl:function name="mk:country-code">
    <xsl:param name="id"/>
    <xsl:value-of select="replace($id,'^PressMint-(.*?)(?:-en)?(?:.ana)?$','$1')"/>
  </xsl:function>

  <xsl:function name="mk:corresp">
    <xsl:param name="node"/>
    <xsl:choose>
      <xsl:when test="$node[not(@corresp) or not(contains(concat(' ',@corresp),' PressMint:')) or contains(concat(' ', @corresp,' '),concat(' ',$corpusType,' '))]">
