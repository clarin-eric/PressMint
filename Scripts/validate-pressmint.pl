...
sub usage
{
    print STDERR ("Usage: validate-pressmint.pl <SchemaDirectory> '<InputDirectories>'\n");
    print STDERR ("       Produces a validation report on PressMint XML files in the <InputDirectories>:\n");
    print STDERR ("       * validation for illegal characters (like soft hyphen, PUA)\n");
    print STDERR ("       * validation against PressMint RNG schemas in <SchemaDirectory> (with jing)\n");
    print STDERR ("       * validation against PressMint ODD schema in <SchemaDirectory> (with jing)\n");
    print STDERR ("       * link (IDREF) checking (with saxon, check-links.xsl)\n");
    print STDERR ("       * content checking (with saxon, validate-pressmint.xsl)\n");
...
$Compose = "$Bin/pressmint-composite-teiHeader.xsl";
...
foreach my $inDir (glob "$inDirs") {
    ...
    foreach $inFile (glob "$inDir/*.xml") {
        if ($inFile =~ m|(PressMint-[A-Z]{2}(?:-[A-Z0-9]{1,3})?(?:-[a-z]{2,3})?\.xml)|) {
            $fileName = $1;
            $rootFile = $inFile;
        }
        elsif ($inFile =~ m|(PressMint-[A-Z]{2}(?:-[A-Z0-9]{1,3})?(?:-[a-z]{2,3})?\.ana\.xml)|) {
            $fileNameAna = $1;
            $rootAnaFile = $inFile
        }
    }
