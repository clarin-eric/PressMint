...
$scriptFactorise  = "$Bin/pressmint-factorize-teiHeader.xsl";
$scriptTaxonomy= "$Bin/pressmint-init-taxonomy.xsl";

binmode(STDERR, 'utf8');

if ($inDir =~ /PressMint-[A-Z-]+\.TEI/) {$corpDirs = $inDir}
else {$corpDirs = "$inDir/PressMint-*.TEI*"}
foreach $corpDir (sort glob($corpDirs)) {
    my $param = '';
    ($country, $anaSuffix) = $corpDir =~ /PressMint-([A-Z-]+)\.TEI(\..+)?/
	...
	#For .ana we will also need the .TEI root file
        my $teiRoot = "$corpDir/PressMint-$country.xml";
	$teiRoot =~ s|\Q$anaSuffix\E||;
        if (-e $teiRoot){$param = " teiRoot=$teiRoot "}
	else {print STDERR "WARN: $teiRoot not found\n"}
    }
    $Prefix = "PressMint-$country-";
    `rm $corpDir/*.xml`;
    $command = "$Saxon outDir=$corpDir prefix=$Prefix $param -xsl:$scriptFactorise $bkpDir/PressMint-$country$anaSuffix.xml";
	...
	$taxonomyFName = "PressMint-taxonomy-$taxonomy.xml";
	...
	$rootFile = "$corpDir/PressMint-$country$anaSuffix.xml";
