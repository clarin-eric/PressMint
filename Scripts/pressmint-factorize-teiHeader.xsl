...
<xsl:stylesheet version="2.0"
  ...
  <xsl:param name="taxonomies">NER topic</xsl:param>
  ...
  <xsl:template match="tei:taxonomy">
    ...
    <xsl:variable name="no_id_change"
                  select=".[starts-with(@xml:id,'PressMint-')
                            and
    ...
    <xsl:variable name="fileid">
      <xsl:choose>
        ...
        <xsl:when test="$is_common">
          <xsl:value-of select="concat('PressMint-',local-name(),@xml:id/concat('-',.))" />
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:variable name="interfix">
      <xsl:if test="ends-with(base-uri(),'.ana.xml')
                   ...
                   and not(contains(concat($noAna,' ',replace($noAna,'PressMint-',$pref)),concat($fileid,'.xml')))
  ...
  <xsl:template match="tei:classDecl/xi:include">
