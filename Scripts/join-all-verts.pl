#!/usr/bin/env perl
# Join all PressMint vert files into one, in reverse chronological order
...
foreach my $countryCode (split(/[, ]+/, $countryCodes)) {
    ...
    $inVertDir = "$inDir/PressMint-$countryCode.vert";
...
foreach $key (reverse sort keys %files) {
	...
	while (<IN>) {
	    #Add corpus attribute to paragraphs and notes for easier filtering in joint corpus
