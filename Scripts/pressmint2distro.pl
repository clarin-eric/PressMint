...
sub usage {
    ...
    print STDERR ("    Prepares PressMint corpora for distribution.\n");
    print STDERR ("    <Codes> is the list of country codes of the corpora to be processed.\n");
    print STDERR ("    <Schema> is the directory where PressMint RNG schemas are.\n");
    print STDERR ("    <Docs> is the directory where PressMint README files are.\n");
    ...
    print STDERR ("    <Input> is the directory where PressMint-XX.TEI/ and PressMint-XX.TEI.ana/ are.\n");
    ...
    print STDERR ("    ./pressmint2distro.pl -all -novalid -codes 'BE ES' \\\n");
    print STDERR ("      -schema ../Schema -docs My/Docs/ -in Originals/ -out Final/  \\\n");
    print STDERR ("      2> PressMint.ana.log\n");
}
...
$taxonomy{'PressMint-taxonomy-topic'}                = "$taxonomyDir/PressMint-taxonomy-topic.xml";
$taxonomy{'PressMint-taxonomy-NER.ana'}              = "$taxonomyDir/PressMint-taxonomy-NER.ana.xml";
...
$scriptRelease = "$Bin/pressmint2release.xsl";
$scriptCommon  = "$Bin/pressmint-add-common-content.xsl";
$scriptTaxonomy= "$Bin/pressmint-init-taxonomy.xsl";
$scriptPolish  = "$Bin/polish-xml.pl";
$scriptValid   = "$Bin/validate-pressmint.pl";
$scriptSample  = "$Bin/corpus2sample.xsl";
$scriptTexts   = "$Bin/pressmintp-tei2text.pl";
$scriptVerts   = "$Bin/pressmintp-tei2vert.pl";
$scriptConls   = "$Bin/pressmintp2conllu.pl";
$scriptMetas   = "$Bin/pressmintp-tei2meta.pl";
...
unless ($countryCodes) {
    ...
    print STDERR "For help: pressmint2distro.pl -h\n";
    exit
}
foreach my $countryCode (split(/[, ]+/, $countryCodes)) {
	...
	$vertRegi = 'pressmint' . $Version . '_' . lc $countryCode;
	$vertRegi =~ s/\.//g;   #e.g. 3.1 -> 31, so we will get e.g. pressmint31_at
	$vertRegi =~ s/-/_/g;  #e.g. pressmint31_es-ct.regi to pressmint31_es_ct
        # Remove -en suffix as we don't have pressmint99_xx_en registry files
	...
	#For some reason both PressMint-XX_YYY-MM-DD-meta-en.tsv and PressMint-XX_YYY-MM-DD.ana-meta-en.tsv
	    ...
	    if (-e $taxonomy{$taxonomy}) {
                if (exists($country2lang{$Country})) {
                    ...
                    die "FATAL ERROR: Can't find mapping between country code and language: ".
                        "pls. add \$country2lang{'$Country'} to pressmint2distro.pl!\n"
                }
	    }
...
sub dirify {
    my $MAX = 1;  #In PressMint we always put them in year directories
    my $inDir = shift;
    my @files = glob("$inDir/*");
    if (scalar @files > $MAX) {
	foreach my $file (@files) {
	    if (my ($year) = $file =~ m|PressMint-.+?_(\d\d\d\d)|) {
...
sub cp_readme_top {
    ...
    # sample: # Samples of the PressMint-XX corpus
    # en-smp: # Samples of the PressMint-XX corpus (translation to English)
    # TEI:    # Corpus of historical newspapers PressMint-XX
    # ana:    # Linguistically annotated corpus of historical newspapers PressMint-XX.ana
    # en-TEI: # Corpus of historical newspapers, PressMint-XX-en (translation to English)
    # en-ana: # Linguistically annotated corpus of historical newspapers PressMint-XX-en.ana (translation to English)
	...
	if (m|^# Samples|) {
	    ...
	    if    ($type =~ /sample/i) {print OUT "# Samples of the PressMint-$countryCode corpus"}
	    elsif ($type =~ /tei/i)    {print OUT "# Corpus of historical newspapers PressMint-$countryCode"}
	    elsif ($type =~ /ana/i)    {print OUT "# Linguistically annotated corpus of historical newspapers PressMint-$countryCode"}
